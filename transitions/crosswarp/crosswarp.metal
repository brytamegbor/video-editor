...
// Cross Warp (gl-transitions, by Eke Péter) — both clips warp toward each other
// as they cross. Full wrapped kernel: byte-compatible with
// TransitionMetalProcessor.TransitionUniforms (four floats + a float4). The
// kernel name MUST match the descriptor's `kernel` field.
    ...
    sampler _s)
{
    ...
    return mix(getFromColor((uv - 0.5) * (1.0 - x) + 0.5),
               getToColor((uv - 0.5) * x + 0.5), x);
}

kernel void trx_crosswarp(
